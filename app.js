...
async function fetchMetar(icao) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error("NOAA METAR request failed.");
    }
    ...
    if (!response.ok) {
      throw new Error("Unable to reach the METAR service.");
    }
    const text = (await response.text()).trim();
    if (!text) {
      return null;
...
async function fetchTaf(icao) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error("NOAA TAF request failed.");
    }
    const text = (await response.text()).trim();
    if (!text) {
      return null;
    ...
    const response = await fetch(proxyUrl, { cache: "no-store" });
    if (!response.ok) {
      throw new Error("Unable to reach the TAF service.");
    }
    const text = (await response.text()).trim();
    if (!text) {
      return null;
...
async function loadAirports() {
  ...
  if (!state.airportsPromise) {
    ...
    state.airportsPromise = fetch(OURAIRPORTS_URL, { cache: "force-cache" })
      .then((response) => {
        if (!response.ok) {
          throw new Error("Airport database not reachable.");
        }
...
async function loadRunways() {
  ...
  if (!state.runwaysPromise) {
    state.runwaysPromise = fetch(RUNWAYS_URL, { cache: "force-cache" })
      .then((response) => {
        if (!response.ok) {
          throw new Error("Runway database not reachable.");
        }
        return response.text();
      })
